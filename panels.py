...
def export_panel_attributes(layout, operator, is_file_browser):
    ...
    if body:
        ...
        for index, item in enumerate(operator.vertex_format):
            ...
            group = row.row(align=True)
            opt_move_up = group.operator("export_scene.move_up_attribute_operator", text="", icon='TRIA_UP')
            opt_move_up.id = index
            opt_move_down = group.operator("export_scene.move_down_attribute_operator", text="", icon='TRIA_DOWN')
            opt_move_down.id = index
